...
using Documancer.Application.Common.Interfaces.MediatorWrapper;
...
using Documancer.Infrastructure.Configurations;
using Documancer.Infrastructure.Constants.ClaimTypes;
using Documancer.Infrastructure.Constants.Database;
using Documancer.Infrastructure.Constants.User;
using Documancer.Infrastructure.PermissionSet;
using Documancer.Infrastructure.Persistence.Interceptors;
using Documancer.Infrastructure.Services.MediatorWrapper;
using Documancer.Infrastructure.Services.MultiTenant;
using Documancer.Infrastructure.Services.PaddleOCR;
using Documancer.Infrastructure.Services.Serialization;
...
namespace Documancer.Infrastructure
{
    public static class DependencyInjection
    {
        ...
        private const string POSTGRESQL_MIGRATIONS_ASSEMBLY = "Documancer.Migrators.PostgreSQL";
        ...
        {
            services.AddSettings(configuration).AddDatabase(configuration).AddServices().AddMessageServices(configuration);
        ...
        private static IServiceCollection AddSettings(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.Configure<DatabaseSettings>(configuration.GetSection(DATABASE_SETTINGS_KEY)).AddSingleton(s => s.GetRequiredService<IOptions<DatabaseSettings>>().Value);
        ...
        {
            services.AddScoped<ISaveChangesInterceptor, AuditableEntityInterceptor>().AddScoped<ISaveChangesInterceptor, DispatchDomainEventsInterceptor>();
            ...
            string connectionString)
        {
            switch (dbProvider.ToLowerInvariant())
            {
                case DbProviderKeys.Npgsql:
                {
                    ...
                    return builder.UseNpgsql(connectionString, e => e.MigrationsAssembly(POSTGRESQL_MIGRATIONS_ASSEMBLY)).UseSnakeCaseNamingConvention();
                }
                default:
                {
        ...
        {
            services.AddSingleton<PicklistService>().AddSingleton<IPicklistService>(sp =>
            {
            ...
            services.AddSingleton<TenantService>().AddSingleton<ITenantService>(sp =>
            {
            ...
            return services.AddSingleton<ISerializer, SystemTextJsonSerializer>()
                ...
                .AddTransient<IDocumentOCRService, DocumentOCRService>();
        }

        private static IServiceCollection AddMessageServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddFluentEmail(defaultFromEmail ?? DEFAULT_FROM_EMAIL).AddRazorRenderer(Path.Combine(Directory.GetCurrentDirectory(), EMAIL_TEMPLATES_PATH)).AddMailKitSender(smtpClientOptions);
        ...
        private static IServiceCollection AddAuthenticationService(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddSingleton<UserService>().AddSingleton<IUserService>(sp =>
            {
