...
namespace Documancer.Infrastructure.Services
{
    public class PDFService : IPDFService
    {
        ...
        public async Task<byte[]> ExportAsync<TData>(IEnumerable<TData> data, Dictionary<string, Func<TData, object?>> mappers, string title, bool landscape)
        {
            ...
            Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header().Text(title).SemiBold().FontSize(16).FontColor(Colors.Black);

                    page.Content().PaddingVertical(5, Unit.Millimetre).Table(table =>
                    {
                        var headers = mappers.Keys.Select(x => x).ToList();
                        var dataList = data.ToList();

                        // Rough fit columns calculation
                        var tableWidth = landscape ? (int)(PageSizes.A4.Landscape().Width - MarginPTs * 2) : (int)(PageSizes.A4.Width - MarginPTs * 2);
        ...
        {
            return container.Border(1).Background(Colors.White).Padding(1, Unit.Millimetre).ShowOnce().AlignMiddle();
        }

        private static IContainer BlockHeader(IContainer container)
        {
            return container.Border(1).Background(Colors.Grey.Lighten3).Padding(1, Unit.Millimetre).AlignCenter().AlignMiddle();
        }
    }
}