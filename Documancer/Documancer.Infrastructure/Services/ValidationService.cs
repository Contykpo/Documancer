using Documancer.Application.Common.Extensions;
...
namespace Documancer.Infrastructure.Services
{
    ...
    public class ValidationService : IValidationService
    {
        ...
        // In order to keep the service as generic as possible, validators are provided by type only when required,
        // an alternative would be creating a IValidationService<TRequest> similar to the ValidationBehaviour<TRequest, TResponse>
        // but that would mean injecting a IValidationService for each type of model to validate in a page.
        ...
        {
            return async (model, propertyName) => await ValidatePropertyAsync((TRequest)model, propertyName);
        }
        ...
        public async Task<IEnumerable<string>> ValidatePropertyAsync<TRequest>(TRequest model, string propertyName,
            CancellationToken cancellationToken = default)
        {
            var validationResult = await ValidateAsync(model, options => { options.IncludeProperties(propertyName); }, cancellationToken);
