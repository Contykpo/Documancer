...
using Documancer.Infrastructure.Constants.Localization;
using Documancer.Server.Hubs;
using Documancer.Server.Services;
using Documancer.Server.Services.Fusion;
using Documancer.Server.Services.Layout;
using Documancer.Server.Services.JsInterop;
using Documancer.Server.Services.Navigation;
using Documancer.Server.Services.Notifications;
using Documancer.Server.Services.UserPreferences;
using Documancer.Server.Middlewares;
using Hangfire;
...
namespace Documancer.Server
{
    ...
    {
        /// <summary>
        /// Adds Documancer.Server UI services to the service collection.
        /// </summary>
        ...
        public static IServiceCollection AddServerUI(this IServiceCollection services, IConfiguration config)
        {
            ...
            services.AddMudServices(config =>
            {
                ...
                // TO-DO: Currently planning on deprecating `PreventDuplicates`, at least to the end dev.
                // However, we may end up wanting to instead set it as internal because the docs project relies on it
                // to ensure that the Snackbar always allows duplicates.
                // Warning is disabled for now becauset he project is set to treat warnings as errors.
...
#pragma warning restore 0618
            ...
            services.AddScoped<IApplicationHubWrapper, ServerHubWrapper>().AddSignalR(options => options.MaximumReceiveMessageSize = 64 * 1024);
        ...
        public static WebApplication ConfigureServer(this WebApplication app, IConfiguration config)
        {
            ...
            else
            {
                ...
                // We may want to change this for production scenarios: https://aka.ms/aspnetcore-hsts.
                app.UseHsts();
            }
