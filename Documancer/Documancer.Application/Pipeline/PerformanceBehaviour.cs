...
namespace Documancer.Application.Pipeline
{
        ...
        where TRequest : IRequest<TResponse>
    {
        ...
        public PerformanceBehaviour(ILogger<PerformanceBehaviour<TRequest, TResponse>> logger, ICurrentUserService currentUserService)
        {
            _logger = logger;
        ...
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
        {
            ...
            if (elapsedMilliseconds > 500)
            {
                ...
                _logger.LogWarning("Long-running request detected: {RequestName} ({ElapsedMilliseconds}ms) {Request} by {UserName}", requestName, elapsedMilliseconds, request, userName);
            }
