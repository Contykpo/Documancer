...
namespace Documancer.Application.Extensions
{
    public class CustomHttpHandler (
        ...
        IAccountServices accountServices) : DelegatingHandler
    {
        protected async override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                bool registerUri = request.RequestUri!.AbsoluteUri.Contains(Constant.RegisterRoute);
                ...
                if (loginUrl || registerUri || refreshTokenUrl || adminCreateUrl)
                {
                ...
                if (result.StatusCode == HttpStatusCode.Unauthorized)
                {
                    ...
                    var newJwtToken = await GetRefreshToken(tokenModel.Token!);
